		    ...
		    )
{
  ...
  int                           my_slice, slice_offset, slice_row, orow=0;
  int                           offset, size, i;
  double                        t1, t2;
  int                           count;

  for (my_slice=0, idim=0; idim<ndim; idim++)
    if (slice[idim] != -1) { my_slice = 1; break; }
  if (!my_slice)
...
/*- Node i identifies portion of local memory belonging to row. -------------*/
	...
	if (slice[0] != -1) slice_offset = slice[0]-start[0];
      }
...
/*- On node 0, for each of nnodes messages containing portion sizes ---------*/
	...
	if (my_slice) {
	  if (slice[0] != -1) {
	    if (fwrite ((char *)local + (offset+slice_offset)*esize, esize, 1,
			stream) != 1)
	      map_error ("Cannot write expected number of elements");
	  } else {
	    if (fwrite ((char *)local + offset*esize, esize, count, stream)
		!= count)
	      map_error ("Cannot write expected number of elements");
	  }
	}
